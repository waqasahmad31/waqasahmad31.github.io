    ...
    projects: [
      {
        ...
        summary:
          'A national-scale biometric enrolment and identity platform. I lead full-stack delivery and took enrolment throughput from 4–6K to over 23,000 records an hour.',
        detail: [
          'The platform is an ASP.NET Core 8 REST API with three clients against it — a WPF desktop application for enrolment stations, a Blazor Server administrative client, and an Angular client for bulk import.',
          'It runs across 16 nodes on a client-managed Kubernetes cluster, deployed through GitLab CI/CD, with Production and Disaster Recovery environments that have to behave identically.',
          'I set the security baseline: HashiCorp Vault PKI with AppRole for certificates and secrets, AES-256-GCM/JWE envelope encryption for data at rest, and Keycloak for SSO and MFA.',
          'The ingestion pipeline is built to be resilient rather than merely fast — a RabbitMQ producer/consumer for back-pressure, transactional outbox dispatch so no event is lost between commit and publish, lease-based job recovery so work claimed by a dead node is reclaimed, and Polly retry and circuit-breaker policies on external calls.',
          'Quality and observability standards are mine too: 44 xUnit suites covering end-to-end flows, idempotency and architecture rules, plus OpenTelemetry tracing exported over OTLP into Grafana.',
        ],
        stack: [
          'ASP.NET Core 8',
          'C# / .NET 8',
      ...
      },
      {
        ...
        domain: 'ERP · B2B SaaS',
        summary:
          'A business ERP for growing SMEs covering CRM, inventory, sales, procurement, finance and approvals — my own product, architected and built end-to-end.',
        detail: [
          ...
          'The backend is an ASP.NET Core API on PostgreSQL with Clean Architecture layering — Domain, Application, Infrastructure and API, dependencies pointing inward only — so a module can be reasoned about and tested without standing up the rest of the system.',
          'CQRS separates the two workloads that pull in opposite directions: writes go through the domain model with its invariants, while reads use Dapper projections shaped exactly for the screen. A slow report never forces a compromise in the domain model.',
          'Modules communicate through a transactional outbox rather than reaching into each other’s tables, and Redis hybrid caching keeps reference data — catalogues, tax rates, approval chains — fast across instances.',
          'The Angular frontend consumes an OpenAPI/NSwag-generated client, so a backend contract change breaks the build instead of surfacing at runtime in front of a customer. Authentication is on ASP.NET Core Identity.',
        ],
        stack: [
          'ASP.NET Core',
          'Angular',
  ...
  },
  {
    ...
    projects: [
      ...
      },
      {
        ...
        detail: [
          'Service-per-database across Identity, Catalog, Inventory, Ordering and Notification, with a YARP gateway in front and a Blazor WebAssembly client.',
